...
// Runs daily at 12:05am ET
// Fetches today's Spelling Bee answers from nytbee.com
// Generates a fully SEO-optimised HTML page for spellingbeefinder.com
...
function getDateStrings() {
  ...
  const iso = `${yyyy}-${mm}-${dd}`;                        // 2026-06-23
  const monthNames = ['January','February','March','April','May','June',
    'July','August','September','October','November','December'];
  const month = monthNames[now.getMonth()];
  const day = now.getDate();
  const year = now.getFullYear();
  const friendly = `${month} ${day}, ${year}`;              // June 23, 2026
  const slug = `spelling-bee-${month.toLowerCase()}-${String(day).padStart(2,'0')}-${year}`; // spelling-bee-june-23-2026
  const filename = `${slug}.html`;
  return { iso, friendly, month, day, year, slug, filename, mm, dd };
}
...
// ── Fetch answers from nytbee.com ─────────────────────────────────────────────

async function fetchAnswers(dateStr) {
  const url = `https://nytbee.com/bee_${dateStr}.json`;
  ...
  try {
    ...
    // nytbee.com format:
    // { date, center_letter, outer_letters, answers: [...], pangrams: [...], max_score, genius_level }
    return {
      centerLetter: data.center_letter.toUpperCase(),
      outerLetters: data.outer_letters.map(l => l.toUpperCase()),
      allLetters: [data.center_letter, ...data.outer_letters].map(l => l.toUpperCase()),
      words: data.answers.sort((a, b) => b.length - a.length || a.localeCompare(b)),
      pangrams: data.pangrams.map(p => p.toUpperCase()),
      maxScore: data.max_score,
      geniusScore: data.genius_level,
      totalWords: data.answers.length
    };
  } catch (err) {
    console.error(`nytbee.com fetch failed: ${err.message}`);
    // Fallback: try spellingbeebuddy.com format
    return await fetchAnswersFallback(dateStr);
  }
}

async function fetchAnswersFallback(dateStr) {
  // Secondary source if nytbee.com is unavailable
  const url = `https://www.sbsolver.com/s/${dateStr}`;
  ...
  try {
    ...
    // Parse sbsolver page
    const centerMatch = html.match(/center[^"]*"[^"]*"([A-Z])/i);
    const wordsMatch = html.match(/<li[^>]*>([A-Z]{4,})<\/li>/gi);
    
    if (!wordsMatch) throw new Error('Could not parse fallback source');
    
    const words = wordsMatch.map(m => m.replace(/<[^>]+>/g, ''));
    const center = centerMatch ? centerMatch[1] : '';
    
    return {
      centerLetter: center,
      outerLetters: [],
      allLetters: [...new Set(words.join('').split(''))],
      words: words.sort((a, b) => b.length - a.length),
      pangrams: words.filter(w => {
        const letters = [...new Set(words.join('').split(''))];
        return letters.every(l => w.includes(l));
      }),
      maxScore: words.reduce((sum, w) => sum + (w.length === 4 ? 1 : w.length), 0),
      geniusScore: 0,
      totalWords: words.length
    };
  } catch (err) {
    console.error(`Fallback also failed: ${err.message}`);
    throw new Error('All data sources unavailable — skipping today');
  }
}

// ── HTML Generation ───────────────────────────────────────────────────────────

function buildWordsByLength(words, allLetters) {
  const byLen = {};
  words.forEach(word => {
    const { pts, isPangram } = wordPoints(word, allLetters);
    if (!byLen[word.length]) byLen[word.length] = [];
    byLen[word.length].push({ word: word.toUpperCase(), pts, isPangram });
  });
  return byLen;
}

function wordChipsHtml(wordsInGroup) {
  return wordsInGroup.map(({ word, pts, isPangram }) => {
    const cls = isPangram ? 'wchip pangram' : 'wchip';
    return `<div class="${cls}">${word}${isPangram ? ' \u2605' : ''} <span class="wpts">${pts}</span></div>`;
  }).join('\n            ');
}

function generateHtml(dates, answers) {
  const { friendly, month, day, year, slug, iso } = dates;
  const { centerLetter, outerLetters, allLetters, words, pangrams, maxScore, geniusScore, totalWords } = answers;
  
  const byLength = buildWordsByLength(words, allLetters);
  ...
  // Word sections by length
  const wordSections = lengths.map(len => {
    const label = `${len} Letter${len !== 1 ? 's' : ''}`;
    return `
          <div class="length-group">
            <div class="length-label">${label}</div>
            <div class="word-grid">
              ${wordChipsHtml(byLength[len])}
            </div>
          </div>`;
  }).join('\n');

  // Prev/next day links (just slugs — archive handles the actual links)
  ...
  const metaDesc = `Complete NYT Spelling Bee answers for ${friendly}. Center letter ${centerLetter}, ${totalWords} valid words, pangram: ${primaryPangram}. Full word list sorted by length, free, no login.`;
  const ogDesc = `All ${totalWords} NYT Spelling Bee answers for ${friendly}. Center ${centerLetter}, pangram ${primaryPangram}. Sorted by length, point values shown. Free.`;
...
<meta name="keywords" content="spelling bee ${month.toLowerCase()} ${day} ${year} answers, nyt spelling bee ${month.toLowerCase()} ${day} answers, spelling bee answers today ${year}, spelling bee ${month.toLowerCase()} ${day} pangram, nyt spelling bee answers ${month.toLowerCase()} ${year}, spelling bee answer list today, spelling bee ${centerLetter.toLowerCase()} center, spelling bee pangram today">
...
{"@context":"https://schema.org","@graph":[{"@type":"Article","headline":"Spelling Bee ${friendly} Answers","datePublished":"${iso}","dateModified":"${iso}","author":{"@type":"Organization","name":"SpellingBeeFinder"},"publisher":{"@type":"Organization","name":"SpellingBeeFinder","url":"https://www.spellingbeefinder.com"},"description":"${metaDesc}"},{"@type":"FAQPage","mainEntity":[{"@type":"Question","name":"What is the Spelling Bee answer for ${friendly}?","acceptedAnswer":{"@type":"Answer","text":"The NYT Spelling Bee for ${friendly} has ${totalWords} valid words. The center letter is ${centerLetter} and the pangram is ${primaryPangram}. The maximum possible score is ${maxScore} points. Genius rank requires ${geniusScore} points (70% of maximum)."}},{"@type":"Question","name":"What is the Spelling Bee pangram for ${friendly}?","acceptedAnswer":{"@type":"Answer","text":"The pangram for the NYT Spelling Bee on ${friendly} is ${primaryPangram}. It uses all 7 letters in the puzzle at least once and is worth ${primaryPangram.length + 7} points (${primaryPangram.length} for letter count plus 7 pangram bonus)."}},{"@type":"Question","name":"How many words are in the Spelling Bee for ${friendly}?","acceptedAnswer":{"@type":"Answer","text":"The NYT Spelling Bee for ${friendly} has ${totalWords} valid words. The maximum possible score is ${maxScore} points. Genius rank requires ${geniusScore} points. The center letter is ${centerLetter} and all words must include it."}},{"@type":"Question","name":"What is the Genius score for Spelling Bee ${friendly}?","acceptedAnswer":{"@type":"Answer","text":"The Genius threshold for the NYT Spelling Bee on ${friendly} is ${geniusScore} points, which is 70% of the maximum possible score of ${maxScore}. Queen Bee requires all ${totalWords} words for the full ${maxScore} points."}},{"@type":"Question","name":"What letters are in the Spelling Bee for ${friendly}?","acceptedAnswer":{"@type":"Answer","text":"The NYT Spelling Bee for ${friendly} uses the letters ${allLetters.join(', ')}. The required center letter is ${centerLetter}, which must appear in every valid word. All words must be at least 4 letters and use only these 7 letters, which can repeat freely."}}]}]}
</script>
<style>
:root{--amber:#b45309;--bee:#f59e0b;--cream-border:#fde68a;--s900:#0f172a;--s800:#1e293b;--s700:#334155;--s600:#475569;--s500:#64748b;--s400:#64748b;--s300:#cbd5e1;--s200:#e2e8f0;--s100:#f1f5f9;--s50:#f8fafc;--green:#059669;--wh:#ffffff;}
...
.pangram-star{font-size:28px;flex-shrink:0;}
...
.wchip{padding:7px 13px;border:1px solid var(--s200);border-radius:9px;background:var(--wh);font-size:13px;font-weight:600;color:var(--s700);display:flex;align-items:center;gap:6px;cursor:default;}
...
.nav-date-btn{flex:1;padding:11px 16px;background:var(--wh);border:1.5px solid var(--s200);border-radius:11px;text-decoration:none;font-size:14px;font-weight:600;color:var(--s700);transition:all .15s;text-align:center;}
...
.sbn-em{background:var(--s300);color:var(--s700);}
...
@media(max-width:768px){
  ...
  .word-grid{gap:6px;}
  .wchip{font-size:13px;padding:6px 10px;}
}
</style>
</head>
<body>
...
<div class="page">
  ...
  <div class="breadcrumb">
    <a href="/index.html">Home</a> &rsaquo; <a href="/answers.html">Daily Answers</a> &rsaquo; ${friendly}
  </div>
  ...
  <div class="date-hero">
    ...
    <div class="date-hero-text">
      ...
      <p>All ${totalWords} valid words for today&rsquo;s NYT Spelling Bee puzzle, sorted by length with point values. Center letter: <strong>${centerLetter}</strong>.</p>
    </div>
  </div>
  ...
  <div class="pangram-card">
    ...
    <div>
      ...
      ${extraPangrams.length ? `<div class="extra-pangrams">Also: ${extraPangrams.join(', ')}</div>` : ''}
    </div>
  </div>
  ...
  <!-- CTA bridge to solver -->
  <div class="cta-card">
    <div>
      <h3>Want to Find Words You Missed?</h3>
      <p>Load today&rsquo;s letters into our free solver and search without spoilers using Hints Mode.</p>
    </div>
  ...
  <div class="faq-section">
    ...
    <div class="faq-item"><button class="faq-q" onclick="tFaq(this)" aria-expanded="false">What is the Spelling Bee answer for ${friendly}?<span class="faq-chev">\u2964</span></button><div class="faq-a">The NYT Spelling Bee for ${friendly} has ${totalWords} valid words. The center letter is ${centerLetter} and the pangram is ${primaryPangram}. Maximum possible score is ${maxScore} points. Genius rank requires ${geniusScore} points (70% of maximum).</div></div>
    <div class="faq-item"><button class="faq-q" onclick="tFaq(this)" aria-expanded="false">What is the Spelling Bee pangram for ${friendly}?<span class="faq-chev">\u2964</span></button><div class="faq-a">The pangram for ${friendly} is ${primaryPangram}. It uses all 7 letters at least once and is worth ${primaryPangram.length + 7} points (${primaryPangram.length} for length plus 7 pangram bonus). ${extraPangrams.length ? 'There is also a second pangram: ' + extraPangrams.join(', ') + '.' : 'It is the only pangram in today\'s puzzle.'}</div></div>
    <div class="faq-item"><button class="faq-q" onclick="tFaq(this)" aria-expanded="false">How many words are in the Spelling Bee for ${friendly}?<span class="faq-chev">\u2964</span></button><div class="faq-a">The NYT Spelling Bee for ${friendly} has ${totalWords} valid words. Maximum score is ${maxScore} points and Genius requires ${geniusScore} points. The center letter ${centerLetter} must appear in every valid word, which must be at least 4 letters long.</div></div>
    <div class="faq-item"><button class="faq-q" onclick="tFaq(this)" aria-expanded="false">What is the Genius score for Spelling Bee ${friendly}?<span class="faq-chev">\u2964</span></button><div class="faq-a">Genius threshold for ${friendly} is ${geniusScore} points, which is 70% of the maximum possible ${maxScore} points. Queen Bee requires all ${totalWords} words for the full ${maxScore} points. Finding the pangram ${primaryPangram} alone gives you ${primaryPangram.length + 7} points toward Genius.</div></div>
    <div class="faq-item"><button class="faq-q" onclick="tFaq(this)" aria-expanded="false">What letters are in the Spelling Bee for ${friendly}?<span class="faq-chev">\u2964</span></button><div class="faq-a">The NYT Spelling Bee for ${friendly} uses the letters ${allLetters.join(', ')}. The required center letter is ${centerLetter}. All valid words must include ${centerLetter}, be at least 4 letters long, and use only these 7 letters which can repeat freely.</div></div>
  </div>
  ...
  <div class="share-strip" aria-label="Share this page">
    ...
    <div class="share-btns">
      <button class="sbn sbn-native" onclick="if(navigator.share){navigator.share({title:'Spelling Bee ${friendly} Answers',url:'https://www.spellingbeefinder.com/answers/${slug}.html'})}" aria-label="Share">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
      </button>
      <button class="sbn sbn-copy" id="sbn-copy-${slug}" onclick="navigator.clipboard.writeText('https://www.spellingbeefinder.com/answers/${slug}.html').then(function(){var b=document.getElementById('sbn-copy-${slug}');b.style.background='#059669';b.style.color='#fff';setTimeout(function(){b.style.background='';b.style.color=''},2500)})" aria-label="Copy link">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
      </button>
      <a class="sbn sbn-fb" href="https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fwww.spellingbeefinder.com%2Fanswers%2F${slug}.html" target="_blank" rel="noopener" aria-label="Facebook">
      ...
      <a class="sbn sbn-tw" href="https://twitter.com/intent/tweet?url=https%3A%2F%2Fwww.spellingbeefinder.com%2Fanswers%2F${slug}.html&text=Spelling%20Bee%20${friendly.replace(/ /g,'%20')}%20Answers" target="_blank" rel="noopener" aria-label="X">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.259 5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </a>
      <a class="sbn sbn-wa" href="https://api.whatsapp.com/send?text=Spelling%20Bee%20${friendly.replace(/ /g,'%20')}%20Answers%20https%3A%2F%2Fwww.spellingbeefinder.com%2Fanswers%2F${slug}.html" target="_blank" rel="noopener" aria-label="WhatsApp">
      ...
      <a class="sbn sbn-em" href="mailto:?subject=Spelling%20Bee%20${friendly.replace(/ /g,'%20')}%20Answers&body=https%3A%2F%2Fwww.spellingbeefinder.com%2Fanswers%2F${slug}.html" aria-label="Email">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" aria-hidden="true"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
      </a>
    </div>
  </div>
...
<footer>
  <div class="foot-inner">
    ...
    <div class="foot-col">
      <h4>Tools</h4>
      <a href="/index.html">Spelling Bee Solver</a>
      <a href="/answers.html">Daily Answers Archive</a>
      <a href="/index.html">Hints Mode</a>
      <a href="/index.html">Practice Mode</a>
    </div>
    <div class="foot-col">
      <h4>Resources</h4>
      <a href="/spelling-bee-solver.html">Solver Guide</a>
      <a href="/spelling-bee-words.html">Spelling Bee Words</a>
      <a href="/spelling-bee-buddy.html">Spelling Bee Buddy</a>
      <a href="/gear.html">Books &amp; Gear</a>
    </div>
    <div class="foot-col">
      <h4>Legal</h4>
...
<script>
function tFaq(btn){
  var a=btn.nextElementSibling,c=btn.querySelector('.faq-chev');
  var o=a.classList.contains('open');
  a.classList.toggle('open',!o);c.classList.toggle('open',!o);
...
async function main() {
  ...
  // Check if page already exists (don't overwrite)
  ...
  let answers;
  try {
    answers = await fetchAnswers(dates.iso);
  } catch (err) {
    console.error(`Could not fetch answers: ${err.message}`);
    process.exit(1);
  }

  console.log(`Fetched: ${answers.totalWords} words, pangram: ${answers.pangrams[0]}`);
  ...
  // Save metadata for archive update
  const meta = {
    date: dates.iso,
    friendly: dates.friendly,
    slug: dates.slug,
    filename: dates.filename,
    centerLetter: answers.centerLetter,
    pangram: answers.pangrams[0] || '',
    totalWords: answers.totalWords,
